 ...
 */
describe('My Banking app For iOS', () => {
  it('should be able to register and use BioMetrics', async () => {
     ...
     */
    //
    // Now wait for the biometrics modal for iOS to be shown
    // iOS can have TouchID or FaceID, so use that in the selector
    ...
    await $(iosBiometricsModalSelector).waitForDisplayed();
...
async function signOut() {
  const iosSelector = '-ios class chain:**/XCUIElementTypeButton[`label == "Sign Out"`]';
  await $('~Settings Screen-screen').waitForDisplayed();
  await $('~Sign Out-menu-item').click();
  await $(iosSelector).waitForDisplayed();
  await $(iosSelector).click();
